...
class NewsFeedRepositoryImpl(
    ...
    override suspend fun getNewActivities(): Flow<List<NewActivity>> {
        return myActivitiesDataStore.getAllActivities().map { myActivies ->
            mutableListOf<NewActivity>().apply {
                addAll(myActivies)
                friendsDataStore.getFriendById("69").map { friend ->
                    friend?.let { batman69 ->
                        add(
                            NewActivity(
                ...
                friendsDataStore.getFriendById("70").map { friend ->
                    friend?.let { batman70 ->
                        add(
                            NewActivity(
                ...
                friendsDataStore.getFriendById("720").map { friend ->
                    friend?.let { superman720 ->
                        add(
                            NewActivity(
